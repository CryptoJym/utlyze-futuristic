...
app.get('/tools', async (_req: Request, res: Response) => {
  try {
    if (BACKEND === 'http') {
      if (!MCP_BASE_URL) throw new Error('MCP_BASE_URL not set');
      const url = `${MCP_BASE_URL.replace(/\/$/, '')}/tools`;
      const headers: Record<string, string> = { 'content-type': 'application/json' };
      if (MCP_AUTH_HEADER) headers['authorization'] = MCP_AUTH_HEADER;
    ...
    if (BACKEND === 'stdio') {
      if (!MCP_STDIO_CMD) throw new Error('MCP_STDIO_CMD not set');
...
async function runStdio(inputJson: string): Promise<string> {
